    ...
    if (data[0] == 0XF0 && data[1] == 0XF0) // tuning data
    {
      ...
      if (controlData.checksum == checksum && isSSID(controlData.ssid))
      {
        ...
        if (controlData.roll == trimFlag && controlData.pitch == trimFlag && controlData.throttle == trimFlag && controlData.yaw == trimFlag) {
          ...
          // control //
          ref_t=controlData.throttle;
          sentControlcommand(controlData.roll + Trim_value[0], controlData.pitch + Trim_value[1] ,(int8_t)Input+zxcv, controlData.yaw + Trim_value[2]);
...
uint8_t setPIDgain(void)
{
  ...
  yaw_data_buffer[2] = (uint8_t)(tuningData[0].ki >> 8);
  yaw_data_buffer[3] = (uint8_t)(tuningData[0].ki);
  ...
  data_buffer[2] = yaw_data_buffer[2];
  data_buffer[3] = yaw_data_buffer[3];
  data_buffer[4] = yaw_data_buffer[4];
  data_buffer[5] = yaw_data_buffer[5];
